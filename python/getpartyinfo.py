...
#asks the party ID of which you want to retrieve parti info
eori_id = input("Party ID?\n EU.EORI.NL: ")
party_id = "EU.EORI.NL" + eori_id

#request the party info from the Scheme Owner, using the previously received access_token
s = requests.get("https://scheme.isharetest.net/parties/"+party_id, headers =
                 { "Authorization": access_token})

#prints the info that's at party_info in the Scheme Owner response
json_s = s.json()
cap_token = json_s["party_token"]
dec_cap_token = jwt.decode(cap_token, verify=False)
info = dec_cap_token["party_info"]
